...
plt.text(50, np.mean(rewards) + 15, "Average Reward: " + str(np.mean(rewards)), color='tab:blue', fontsize=15, withdash=True)
plt.legend([rew], ["Rewards"], loc='best')
plt.xlabel('Episodes')
plt.ylabel('Rewards')
plt.show()
fig.savefig('./plots/reward_deep_ga_test_'+str(WORLD_DIM)+'.png')
